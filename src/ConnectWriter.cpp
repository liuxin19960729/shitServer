...
void ConnectWriter::EntireWriteWhenEmpty(shared_ptr<char> buff, streamsize len)
{
    char *s = buff.get();
    ...
    if ((n > 0 && n < len) || (n < 0 && errno == EAGAIN))
    {
        ...
        obj->len = len;
        objs.push_back(obj);
        // ShitNet::Inst()->ModifyEvent(fd, true);
        return;
    }
    //情况1-3：真的发生错误
...
void ConnectWriter::EntireWriteWhenNotEmpty(shared_ptr<char> buff, streamsize len)
{
    auto obj = make_shared<WriteObject>();
    obj->start = 0;
    obj->buff = buff;
...
void ConnectWriter::EntireWrite(shared_ptr<char> buff, streamsize len)
{
    if (isClosing)
    {
...
//返回值:是否完整的写入了一条
bool ConnectWriter::WriteFrontObj()
{
    //没待写数据
    if (objs.empty())
    {
...
void ConnectWriter::OnWriteable()
{
...
void ConnectWriter::LingerClose()
{
    // if (isClosing)
    // {
    //     return;
